...
def main(**kwargs):
    # load credentials
    bearer_token = None
    with open("/home/kduskin/academic_credentials_v2.json", "rt") as f:
    ...
    # get the list of ids
    if kwargs["tweet_ids"] is not None:
        if type(kwargs["tweet_ids"])==str:
            source = open(kwargs["tweet_ids"],"r").readlines()
        elif type(kwargs["tweet_ids"])==list:
            source = kwargs["tweet_ids"]
    else:
        source = sys.stdin
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tweet_ids",help="file containing tweet ids to hydrate, one per line (otherwise use standard in")
